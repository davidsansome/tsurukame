...
void EnsureInitialized() {
  dispatch_once(&sOnceToken, ^{
    ...
    NSArray<NSURL *> *urls = [mainBundle URLsForResourcesWithExtension:nil
                                                          subdirectory:kFontDirectory];
    ...
    for (NSURL *url in urls) {
      ...
      NSPredicate *fontNamePredicate =
          [NSPredicate predicateWithFormat:@"fontName == %@", newFont.fontName];
      NSArray<TKMFont *> *filteredArray =
          [fontsFromDefaults filteredArrayUsingPredicate:fontNamePredicate];
...
+ (TKMFont *)getRandomFontToRender:(NSString *)text {
  NSPredicate *fontPredicate =
      [NSPredicate predicateWithBlock:^BOOL(TKMFont *font, NSDictionary *_) {
        return font.enabled && [TKMFontLoader font:font canRender:text];
      }];

  NSArray<TKMFont *> *availableFonts =
      [[TKMFontLoader getLoadedFonts] filteredArrayUsingPredicate:fontPredicate];
