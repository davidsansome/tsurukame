...
@objc extension TKMSubject {
  ...
  fileprivate func readingsFilteredByPrimary(primary: Bool) -> [TKMReading] {
    var ret = [TKMReading]()
  ...
  var commaSeparatedReadings: String {
    var strings = [String]()
    let readingArray = readingsArray as! [TKMReading]
    for reading in readingArray {
  ...
  var commaSeparatedPrimaryReadings: String {
    var strings = [String]()
...
@objc extension TKMVocabulary {
  ...
  fileprivate func isPartOfSpeech(part: TKMVocabulary_PartOfSpeech) -> Bool {
  ...
  var isGodanVerb: Bool { isPartOfSpeech(part: TKMVocabulary_PartOfSpeech.godanVerb) }
  var isSuruVerb: Bool { isPartOfSpeech(part: TKMVocabulary_PartOfSpeech.suruVerb) }
  var isNoun: Bool { isPartOfSpeech(part: TKMVocabulary_PartOfSpeech.noun) }
...
@objc extension TKMAssignment {
  ...
  func guruDate() -> Date? {
    ...
    let guruSeconds = Convenience.minGuruTime(level, srsStage: srsStage + 1)
...
@objc extension TKMProgress {
  var reviewFormParameters: String {
    """
  ...
  var lessonFormParameters: String { "keys%%5B%%5D=\(assignment.subjectId)" }
...
@objc extension TKMLevel {
  ...
  var timeSpentCurrent: TimeInterval {
    guard hasUnlockedAt else { return 0 }
